...
ALTER TABLE artifacts ADD CONSTRAINT artifacts_kind_check CHECK (kind IN (
  ...
  'video-render-input','video-manifest','video-mp4','image-asset','image-metadata'
));
ALTER TABLE artifacts ADD CONSTRAINT artifacts_media_type_check CHECK (
  ...
  (kind = 'video-mp4' AND media_type = 'video/mp4') OR
  (kind = 'image-asset' AND media_type IN ('image/png','image/jpeg','image/webp')) OR
  ...
  (kind IN ('research-json','material-json','deck-spec','render-manifest','narration-manifest','tts-manifest','video-render-input','video-manifest','image-metadata') AND media_type = 'application/json')
);
