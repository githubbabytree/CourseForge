...
ALTER TABLE artifacts ADD CONSTRAINT artifacts_kind_check CHECK (kind IN (
  ...
  'design-plan','visual-analysis','qa-report','qa-approval','published-course','image-search-candidates','webppt-package','release-manifest'
));
ALTER TABLE artifacts ADD CONSTRAINT artifacts_media_type_check CHECK (
  ...
  (kind NOT IN ('reveal-html','audio-wav','video-mp4','webppt-package','image-asset','subtitles-vtt','subtitles-srt') AND media_type='application/json')
);
