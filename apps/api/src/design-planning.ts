...
import { DeckSpecV1Schema, DesignPlanV1Schema, StyleProfileV1Schema } from "@courseforge/contracts";
...
export async function planDesign(repository:CourseForgeRepository,blobs:ArtifactBlobStore,provider:DesignProvider,input:{projectId:string;snapshotId:string;materialArtifactId:string;durationMinutes:number;brandAssetIds:string[];jobId?:string;materialContentHash?:string;brandAssetContentHashes?:Record<string,string>}) {
  ...
  const styleMetadata=(await repository.listArtifactMetadata(input.projectId)).filter(item=>item.kind==="visual-style-profile"&&item.configurationVersion===input.snapshotId).sort((a,b)=>b.createdAt.localeCompare(a.createdAt))[0];let styleProfile:ReturnType<typeof StyleProfileV1Schema.parse>|undefined;if(styleMetadata){const bytes=await blobs.get(styleMetadata.artifactId);if(!bytes||bytes.byteLength!==styleMetadata.byteLength||createHash("sha256").update(bytes).digest("hex")!==styleMetadata.contentHash)throw new DesignPlanningError("Style profile integrity check failed");styleProfile=StyleProfileV1Schema.parse(JSON.parse(Buffer.from(bytes).toString("utf8")));}
  const directions=await provider.proposeDirections({title:String(material.value.title),audience:String(material.value.audience),durationMinutes:input.durationMinutes,brandAssets:input.brandAssetIds,...(styleProfile?{styleProfile:styleProfile.tokens}:{})},{runId:crypto.randomUUID(),projectId:input.projectId,configurationVersion:input.snapshotId});
  ...
  const plan=DesignPlanV1Schema.parse({schemaVersion:"1",planId:crypto.randomUUID(),projectId:input.projectId,snapshotId:input.snapshotId,materialArtifactId:input.materialArtifactId,materialContentHash:material.metadata.contentHash,styleProfileBinding:styleMetadata?{artifactId:styleMetadata.artifactId,contentHash:styleMetadata.contentHash}:null,directions:normalized,defaultDirectionId:normalized[0]!.directionId,createdAt:new Date().toISOString()});
  const artifact=await persistContentJsonArtifact({repository,blobStore:blobs,projectId:input.projectId,jobId:input.jobId??crypto.randomUUID(),configurationVersion:input.snapshotId,providerId:provider.metadata.id,kind:"design-plan",value:plan,sourceArtifactIds:styleMetadata?[styleMetadata.artifactId]:[],revision:(await repository.listArtifactMetadata(input.projectId)).filter(item=>item.kind === "design-plan").length+1});
...
export async function buildSelectedDeck(repository:CourseForgeRepository,blobs:ArtifactBlobStore,templates:DesignTemplateStore,provider:DesignProvider,input:{projectId:string;snapshotId:string;planArtifactId:string;directionId?:string;templateId?:string;brandAssetIds:string[];durationMinutes:number;jobId?:string;planContentHash?:string;materialContentHash?:string;templateContentHash?:string|null;brandAssetContentHashes?:Record<string,string>}) {
  ...
  let styleProfile:ReturnType<typeof StyleProfileV1Schema.parse>|undefined;if(plan.styleProfileBinding){const source=await repository.findArtifactMetadata(plan.styleProfileBinding.artifactId),bytes=await blobs.get(plan.styleProfileBinding.artifactId);if(!source||source.contentHash!==plan.styleProfileBinding.contentHash||!bytes||createHash("sha256").update(bytes).digest("hex")!==source.contentHash)throw new DesignPlanningError("Style profile changed after design planning");styleProfile=StyleProfileV1Schema.parse(JSON.parse(Buffer.from(bytes).toString("utf8")));}
  const generated=await provider.buildDeck({title:String(material.title),audience:String(material.audience),durationMinutes:input.durationMinutes,brandAssets:input.brandAssetIds,directionId,directionThemeTokens:direction.themeTokens,...(styleProfile?{styleProfile:styleProfile.tokens}:{}),template:template ? {templateId:template.templateId,contentHash:template.contentHash,themeTokens:template.themeTokens,layoutConstraints:template.layoutConstraints}:undefined,outline:sections.map(section=>section.title),sections},{runId:crypto.randomUUID(),projectId:input.projectId,configurationVersion:input.snapshotId});
  const deck=DeckSpecV1Schema.parse({...generated,designBinding:{planArtifactId:input.planArtifactId,planContentHash:loadedPlan.metadata.contentHash,directionId,templateId:template?.templateId??null,templateContentHash:template?.contentHash??null,brandAssetIds:input.brandAssetIds,brandAssetContentHashes:Object.fromEntries(assets.map(asset=>[asset.assetId,asset.contentSha256])),styleProfileArtifactId:plan.styleProfileBinding?.artifactId??null,styleProfileContentHash:plan.styleProfileBinding?.contentHash??null,usedDefaultDirection:input.directionId === undefined}});
  ...
  const bundle=await createDeckArtifactBuilder(source)(deck,{projectId:input.projectId,jobId:input.jobId??crypto.randomUUID(),revision,configurationVersion:input.snapshotId,providerId:provider.metadata.id,deckSourceArtifactIds:[input.planArtifactId,...(plan.styleProfileBinding?[plan.styleProfileBinding.artifactId]:[])]});
