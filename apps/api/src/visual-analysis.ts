...
export async function runVisualAnalysis(repository: CourseForgeRepository, blobs: ArtifactBlobStore, projectId: string, snapshotId: string, assetIds: string[], options: VisualAnalysisOptions = {}) {
  ...
  if (!prompt) throw new Error("multimodal_prompt_unavailable");
  ...
  const responseSchema={type:"object",additionalProperties:false,required:["summary","ocrHints","chartInsights","risks"],properties:{summary:{type:"string"},ocrHints:{type:"array",items:{type:"string"}},chartInsights:{type:"array",items:{type:"string"}},risks:{type:"array",items:{type:"string"}}}}as const;
  let conclusion:Readonly<Record<string,unknown>>;try{conclusion=(await provider.inspect({prompt:renderedPrompt,assets:assets.map(({asset,bytes})=>({uri:`data:${asset.mediaType};base64,${Buffer.from(bytes).toString("base64")}`,mediaType:asset.mediaType})),responseSchema},{runId:crypto.randomUUID(),projectId,configurationVersion:snapshotId})).observation;}catch(cause){if(cause instanceof Error&&cause.message.includes("exceeds the configured size limit"))throw new Error("multimodal_response_too_large",{cause});throw new Error("multimodal_upstream",{cause});}
