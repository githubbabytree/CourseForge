...
export class PostgresDesignTemplateStore implements DesignTemplateStore {
  ...
  async transition(id:string,operation:"publish"|"deactivate",at:string) { const from=operation === "publish" ? "draft" : "published"; const to=operation === "publish" ? "published" : "inactive"; const result=await this.sql.query<{document:unknown}>("UPDATE design_template_versions SET status=$2,published_at=CASE WHEN $2='published' THEN $3 ELSE published_at END,inactive_at=CASE WHEN $2='inactive' THEN $3 ELSE NULL END,document=jsonb_set(jsonb_set(jsonb_set(document,'{status}',to_jsonb($2::text)),'{publishedAt}',to_jsonb(CASE WHEN $2='published' THEN $3::text ELSE document->>'publishedAt' END)),'{inactiveAt}',to_jsonb(CASE WHEN $2='inactive' THEN $3::text ELSE NULL END)) WHERE template_id=$1 AND status=$4 RETURNING document",[id,to,at,from]); return result.rows[0] ? DesignTemplateVersionV1Schema.parse(result.rows[0].document) : undefined; }
}
